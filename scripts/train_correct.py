...
def train(config: DictConfig):
    ...
    # ---------------------
    callbacks = list()
    callbacks.append(ProgressLoggerCallback(refresh_rate=100))
    ...
    if val_check_interval is not None and check_val_every_n_epoch is not None:
        raise ValueError("Configure at most one of val.check_interval and val.check_every_n_epoch.")
    ...
    ckpt_path = (
        require_checkpoint_path(config.train.ckpt_path, "train.ckpt_path")
        if config.train.ckpt_path is not None else None
    )
    trainer.fit(model=correct_model, ckpt_path=ckpt_path, datamodule=data_module)
